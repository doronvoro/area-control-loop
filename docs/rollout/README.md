...
| 2 | *(backup)* | — | See below. Do not skip. |
| 3 | `01a-fix-report-number.sql` | **yes** | **Run before the bundle.** Standalone — it has its own COMMIT. |
...
| 6 | *(ledger repair)* | yes | Only if the ledger turns out to be meaningful — see below. |

## Production has diverged from this repo

The pre-flight output showed four things the migration history does not explain:

1. **`report_areas.report_number` exists as a plain integer** (`is_identity=NO`).
   No migration here creates it that way — `20260908000000` creates it as
   `GENERATED BY DEFAULT AS IDENTITY`. It was added by hand.
...
- **A recorded migration version does not mean its schema is present.** Do not
  use the ledger, or `db push`, to decide what to run. `00b-followup.sql` query B
  measures how far the ledger has drifted.
- **`ADD COLUMN IF NOT EXISTS` is a trap here.** Because `report_number` already
  exists, that statement is suppressed *in full* — including the identity
  property. The migration would report success and change nothing, leaving every
  future INSERT with a NULL report number. `01a-fix-report-number.sql` converts
  the existing column instead, and must run **before** the bundle.
