...
export const ISSUE_CATEGORIES: Record<IssueCategory, IssueCategoryInfo> = {
  ...
  growerAlias: {
    label: 'שם מגדל שאוחד',
    what: 'שם המגדל שבקובץ רשום במערכת כשם נוסף של מגדל אחר. כך נשמר איחוד שנעשה במסך המגדלים — בלעדיו כל ייבוא היה יוצר מחדש את השם שאוחד, כמגדל נפרד.',
    effect: 'החלקה שויכה למגדל שאליו אוחד השם, וזה גם השם שמוצג עבורה. הקובץ עצמו לא השתנה.',
    action:
      'אם האיחוד נכון — אין צורך בפעולה, וההודעה כאן רק כדי שהשינוי לא יהיה שקט. אם מדובר במגדל נפרד שאוחד בטעות, יש להסיר את השם הנוסף בכרטיס המגדל ולייבא מחדש.',
  },
  taktCount: {
    label: 'מספר טאקטים',
