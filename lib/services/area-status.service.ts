...
export type AreaStatus = 'no_monitoring' | 'all_done' | 'partial' | 'needs_action';
...
export async function getAreasStatus(
  supabase: SupabaseClient,
  options: { customerId?: string | null; isAdmin: boolean }
): Promise<AreaStatusResult[]> {
  const { customerId, isAdmin } = options;
  ...
  if (!monReportAreas || monReportAreas.length === 0) {
    return (areas || []).map((a: any) => buildEmptyStatus(a));
  }
  ...
  const latestActionByArea = await getLatestActionDates(supabase, areaIds);

  // Aggregate per area
  const areaStats = aggregateStats(monitoringReports || [], reportAreaToArea);

  // Build response
  return (areas || []).map((a: any) => {
    const stats = areaStats[a.id];
    if (!stats) return buildEmptyStatus(a);
...
function buildEmptyStatus(area: any): AreaStatusResult {
  return {
    id: area.id,
    name: area.name,
    status: 'no_monitoring',
...
async function getLatestActionDates(
  supabase: SupabaseClient,
  areaIds: string[]
...
function aggregateStats(
  ...
  reportAreaToArea: Record<string, string>
): Record<string, {
  total_findings: number;
  total_treatments: number;
  completed_treatments: number;
  ...
  for (const report of monitoringReports) {
    ...
    stats[aId].total_findings++;

    for (const t of (report.treatments || [])) {
      stats[aId].total_treatments++;
      if (t.action_treatment_id) {
        stats[aId].completed_treatments++;
      }
    }

    if (!stats[aId].last_monitoring || report.created_at > stats[aId].last_monitoring!) {
      stats[aId].last_monitoring = report.created_at;
    }
  }
